...
| Quiescent SQLite generation store | Tested | Optional `ltx_sqlite`; durable empty baseline, host-owned connection gate, typed shared generation access, canonical immutable read-only URI, sidecar rejection, exclusive indeterminate recovery, subprocess crash-point recovery, and a real checksummed SQLite A-to-B-to-C snapshot/incremental chain with growth, shrink, blocked WAL reset and publication, exact active/retained bytes, and semantic integrity checks |
