...
| Quiescent SQLite generation store | Tested | Optional `ltx_sqlite`; explicit state/failure inspection, durable empty baseline, host-owned connection gate, canonical immutable read-only URI, sidecar rejection, exclusive indeterminate recovery, synchronized cross-process shared-lease contention and abrupt abandonment, retained and stale-epoch lease qualification, and SQLite-produced checksummed A-to-B-to-C crash replay at every publication durability boundary with growth, shrink, slot reuse, exact active/retained bytes, and semantic integrity checks |
