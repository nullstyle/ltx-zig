...
digest and reconstructed database digest is a known answer. The optional
`interop` step verifies a fresh Zig snapshot with the exact pinned Go decoder.
It also independently constructs checked merge and deletion inputs plus a
no-checksum chain in Go, runs the pinned Go compactor, byte-compares its outputs
with fresh Zig compaction outputs, and decodes them again with Go. Normal tests
stay network-free, replay the checked-in fuzz corpora, and run a deterministic
